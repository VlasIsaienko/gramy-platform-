...
export default function TournamentDetailPage({ params }: { params: { id: string } }) {
  ...
  function bracketErrorMessage(e: unknown, fallbackKey: string): string {
    if (e instanceof BracketError) return t(`bracketGeneration.errors.${e.code}`, e.params);
    return t(fallbackKey);
  }

  function roundLabel(round: number, isOlympic: boolean, olympicTotalRounds: number): string {
    if (!isOlympic) return t("tournamentDetail.roundLabel", { round });
    const label = olympicRoundLabel(round, olympicTotalRounds);
    return label.kind === "numbered"
      ? t("tournamentDetail.roundLabel", { round: label.round })
      : t(`tournamentDetail.olympicRoundLabels.${label.kind}`);
  }
  ...
  async function loadAll() {
    ...
    if (categoriesRes.error) setError(t("tournamentDetail.errors.loadCategories", { message: categoriesRes.error.message }));
    else setCategories(categoriesRes.data || []);

    if (registrationsRes.error) setError(t("tournamentDetail.errors.loadRegistrations", { message: registrationsRes.error.message }));
    else setRegistrations(registrationsRes.data || []);

    if (playersRes.error) setError(t("tournamentDetail.errors.loadPlayers", { message: playersRes.error.message }));
    else setPlayers(playersRes.data || []);

    if (teamsRes.error) setError(t("tournamentDetail.errors.loadTeams", { message: teamsRes.error.message }));
    else setTeams(teamsRes.data || []);

    if (matchesRes.error) setError(t("tournamentDetail.errors.loadMatches", { message: matchesRes.error.message }));
  ...
  async function handleAddCategory(e: React.FormEvent) {
    ...
    if (error) setError(t("tournamentDetail.errors.addCategory", { message: error.message }));
    else {
      setNewCategoryName("");
  ...
  async function handleAddPlayer(categoryId: string, playerId: string) {
    ...
    if (error) setError(t("tournamentDetail.errors.registerPlayer", { message: error.message }));
    else await loadAll();
  }
  ...
  async function handleRegisterSolo(categoryId: string, playerId: string) {
    ...
    if (regError) { setError(t("tournamentDetail.errors.registerPlayer", { message: regError.message })); return; }
    const { error: teamError } = await supabase.from("teams").insert([{
      tournament_id: tournamentId, category_id: categoryId, player_id_1: playerId, player_id_2: null,
    }]);
    if (teamError) { setError(t("tournamentDetail.errors.createTeam", { message: teamError.message })); return; }
  ...
  async function handleRegisterPair(categoryId: string, playerAId: string, playerBId: string) {
    ...
    if (regError) { setError(t("tournamentDetail.errors.registerPlayers", { message: regError.message })); return; }
    const { error: teamError } = await supabase.from("teams").insert([{
      tournament_id: tournamentId, category_id: categoryId, player_id_1: playerAId, player_id_2: playerBId,
    }]);
    if (teamError) { setError(t("tournamentDetail.errors.createPair", { message: teamError.message })); return; }
    setPendingRegistration((prev) => { const next = { ...prev }; delete next[categoryId]; return next; });
    ...
    if (category && category.match_category !== "singles") {
      ...
      if (team) {
        if (team.player_id_2) {
          ...
          if (teamError) { setError(t("tournamentDetail.errors.updatePair", { message: teamError.message })); return; }
        } else {
          const { error: teamError } = await supabase.from("teams").delete().eq("id", team.id);
          if (teamError) { setError(t("tournamentDetail.errors.deleteTeam", { message: teamError.message })); return; }
        }
      }
    }

    const { error } = await supabase.from("registrations").delete().eq("id", registration.id);
    if (error) { setError(t("tournamentDetail.errors.removePlayer", { message: error.message })); return; }
    await loadAll();
  }
  ...
  async function handleForcePair(categoryId: string, playerAId: string, playerBId: string) {
    ...
    if (mergeError) { setError(t("tournamentDetail.errors.createPair", { message: mergeError.message })); return; }
    ...
    if (teamBWasComplete) {
      ...
      if (error) { setError(t("tournamentDetail.errors.updateFreedPartner", { message: error.message })); return; }
    } else {
      const { error } = await supabase.from("teams").delete().eq("id", teamB.id);
      if (error) { setError(t("tournamentDetail.errors.deleteEmptyTeam", { message: error.message })); return; }
    }
  ...
  async function handleAutoPairAll(categoryId: string) {
    ...
    for (let i = 0; i < pairsCount; i++) {
      ...
      if (updateError) { setError(t("tournamentDetail.errors.formPairs", { message: updateError.message })); return; }
      const { error: deleteError } = await supabase.from("teams").delete().eq("id", b.id);
      if (deleteError) { setError(t("tournamentDetail.errors.formPairs", { message: deleteError.message })); return; }
    }
    await loadAll();
  }

  async function handleDeleteBracket(category: Category) {
    if (!window.confirm(t("tournamentDetail.confirm.deleteBracket"))) return;
    setError(null);
    const { error } = await supabase.from("matches").delete().eq("category_id", category.id);
    if (error) setError(t("tournamentDetail.errors.deleteBracket", { message: error.message }));
    else await loadAll();
  }
  ...
  async function handleGenerateBracket(category: Category) {
    ...
    if (isDoublesLike) {
      ...
      if (missingPlayerIds.length > 0) {
        ...
        if (error) { setError(t("tournamentDetail.errors.createTeams", { message: error.message })); return; }
    ...
    if (tournament.format === "olympic") {
      ...
      try {
        if (mode === "manual") {
          ...
          if (!slots || slots.length !== size || slots.filter((s) => s !== null).length !== participantIds.length) {
            setError(t("bracketGeneration.errors.fillAllSlots"));
        ...
        setError(bracketErrorMessage(e, "bracketGeneration.errors.generic"));
        setGeneratingCategoryId(null);
        return;
      }

      const { error: prefError } = await supabase.from("categories").update({ third_place_match: playThirdPlace }).eq("id", category.id);
      if (prefError) { setError(t("tournamentDetail.errors.savePreferences", { message: prefError.message })); setGeneratingCategoryId(null); return; }
      ...
      if (error) setError(t("tournamentDetail.errors.saveBracket", { message: error.message }));
      else await loadAll();
    ...
    if (tournament.format === "groups") {
      for (const pool of pools) {
        ...
        for (const participantId of pool.participants) {
          ...
          if (error) { setError(t("tournamentDetail.errors.saveGroups", { message: error.message })); setGeneratingCategoryId(null); return; }
        }
      }
    }
  ...
  async function handleMoveToGroup(category: Category, teamId: string, fromGroup: number, toGroup: number) {
    ...
    if (moveError) { setError(t("tournamentDetail.errors.moveParticipant", { message: moveError.message })); return; }

    const { error: deleteError } = await supabase.from("matches").delete().eq("category_id", category.id).in("group_number", [fromGroup, toGroup]);
    if (deleteError) { setError(t("tournamentDetail.errors.recalcMatches", { message: deleteError.message })); return; }
    ...
    if (newMatchRows.length > 0) {
      ...
      if (insertError) { setError(t("tournamentDetail.errors.saveRecalculatedMatches", { message: insertError.message })); return; }
    }
  ...
  async function handleUpdateMaMatch(match: Match, a1: string, a2: string, b1: string, b2: string) {
    ...
    if (new Set(four).size !== 4 || four.some((p) => !p)) {
      setError(t("tournamentDetail.errors.fourDistinctPlayers"));
      return;
    }
    ...
    if (four.some((p) => busy.has(p))) {
      setError(t("tournamentDetail.errors.playerBusyOtherCourt"));
      return;
    }

    const { error: errA } = await supabase.from("teams").update({ player_id_1: a1, player_id_2: a2 }).eq("id", match.team_a_id);
    if (errA) { setError(t("tournamentDetail.errors.updateTeamA", { message: errA.message })); return; }
    const { error: errB } = await supabase.from("teams").update({ player_id_1: b1, player_id_2: b2 }).eq("id", match.team_b_id!);
    if (errB) { setError(t("tournamentDetail.errors.updateTeamB", { message: errB.message })); return; }
  ...
  async function handleUpdateMatch(match: Match, newTeamAId: string, newTeamBId: string) {
    setError(null);
    if (newTeamAId === newTeamBId) { setError(t("tournamentDetail.errors.selfMatch")); return; }
    ...
    if (busy.has(newTeamAId) || busy.has(newTeamBId)) {
      setError(t("tournamentDetail.errors.participantBusy"));
      return;
    }
    ...
    if (tournament?.format === "round_robin" || tournament?.format === "groups") {
      ...
      if (issues.length > 0) {
        const issueLines = issues.map((issue) =>
          issue.type === "doubleBooked"
            ? t("tournamentDetail.integrityIssues.doubleBooked", { round: issue.round })
            : t(`tournamentDetail.integrityIssues.${issue.type}`)
        );
        const proceed = window.confirm(t("tournamentDetail.confirm.roundRobinIntegrityWarning", { issues: issueLines.join("\n") }));
        if (!proceed) return;
      }
    }

    const { error } = await supabase.from("matches").update({ team_a_id: newTeamAId, team_b_id: newTeamBId }).eq("id", match.id);
    if (error) { setError(t("tournamentDetail.errors.saveMatchChanges", { message: error.message })); return; }
    cancelEditMatch();
    await loadAll();
  }
  ...
  async function handleSaveScore(match: Match) {
    ...
    if (!Number.isFinite(scoreA) || !Number.isFinite(scoreB)) {
      setError(t("bracketDisplay.errors.enterScore"));
      return;
    }

    const result = validateMatchScore(scoreA, scoreB);
    if (!result.valid) { setError(t(`bracketDisplay.scoreErrors.${result.errorCode}`)); return; }
    ...
    if (error) { setError(t("tournamentDetail.errors.saveResult", { message: error.message })); return; }
  ...
  async function handleSaveSet(match: Match, setNumber: number) {
    ...
    const scoreB = parseInt(draft?.b ?? "", 10);
    if (!Number.isFinite(scoreA) || !Number.isFinite(scoreB)) {
      setError(t("bracketDisplay.errors.enterSetScore"));
      return;
    }
    ...
    if (insertError) { setError(t("tournamentDetail.errors.saveSet", { message: insertError.message })); return; }
    ...
    if (winner) {
      ...
      if (matchError) { setError(t("tournamentDetail.errors.finishMatch", { message: matchError.message })); return; }
    }
  ...
  async function handleGenerateNextOlympicRound(category: Category) {
    ...
    if (error) setError(t("tournamentDetail.errors.generateRound", { message: error.message }));
    else await loadAll();
  ...
  async function insertCourtsAsMatches(category: Category, roundNumber: number, courts: Court<string>[]): Promise<string | null> {
    ...
    if (teamsError) return t("tournamentDetail.errors.createTeams", { message: teamsError.message });
    ...
    if (matchesError) return t("tournamentDetail.errors.saveMatches", { message: matchesError.message });
    return null;
  }
  ...
  async function handleGenerateMexicanoFirstRound(category: Category) {
    ...
    if (!Number.isFinite(totalRounds) || totalRounds < 1) { setError(t("bracketGeneration.errors.enterRoundsCount")); return; }
    ...
    try {
      ...
      setError(bracketErrorMessage(e, "bracketGeneration.errors.genericRound"));
      setGeneratingCategoryId(null);
      return;
    }

    const { error: roundsError } = await supabase.from("categories").update({ total_rounds: totalRounds }).eq("id", category.id);
    if (roundsError) { setError(t("tournamentDetail.errors.saveRoundsCount", { message: roundsError.message })); setGeneratingCategoryId(null); return; }
  ...
  if (!tournament) {
    return <div className="bg-red-50 text-red-700 rounded-lg px-4 py-3 text-sm">{error || t("tournamentDetail.errors.tournamentNotFound")}</div>;
  }

  return (
    <div>
      <Link href="/admin/tournaments" className="text-sm text-slateGray hover:text-court transition">{t("tournamentDetail.backToTournaments")}</Link>
      ...
      <form onSubmit={handleAddCategory} className="bg-white rounded-xl p-6 shadow-sm mb-6 flex gap-3 items-end">
        <div className="flex-1">
          <label className="text-sm text-slateGray mb-1 block">{t("tournamentDetail.newCategory")}</label>
          <input
            type="text"
            value={newCategoryName}
            onChange={(e) => setNewCategoryName(e.target.value)}
            placeholder={t("tournamentDetail.categoryNamePlaceholder")}
            className="w-full border border-black/10 rounded-lg px-4 py-2.5 outline-none focus:border-court"
          />
        </div>
        <div>
          <label className="text-sm text-slateGray mb-1 block">{t("tournamentDetail.type")}</label>
          <select
          ...
          >
            {MATCH_CATEGORY_VALUES.map((c) => <option key={c} value={c}>{t(`common.matchCategories.${c}`)}</option>)}
          </select>
        </div>
        <div>
          <label className="text-sm text-slateGray mb-1 block">{t("tournamentDetail.scoring")}</label>
          <select
            value={newScoringFormat}
            onChange={(e) => setNewScoringFormat(e.target.value as ScoringFormat)}
            className="border border-black/10 rounded-lg px-4 py-2.5 outline-none focus:border-court bg-white"
          >
            <option value="single_set">{t("tournamentDetail.scoringOptions.single_set")}</option>
            <option value="best_of_3">{t("tournamentDetail.scoringOptions.best_of_3")}</option>
          </select>
        </div>
        <button
          type="submit"
        ...
        >
          {savingCategory ? t("common.actions.saving") : t("tournamentDetail.addCategoryButton")}
        </button>
      </form>

      {categories.length === 0 ? (
        <div className="bg-white rounded-xl p-8 text-center text-slateGray shadow-sm">
          {t("tournamentDetail.noCategories")}
        </div>
      ) : (
              ...
              <div key={category.id} className="bg-white rounded-xl shadow-sm p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xs text-slateGray">{t(`common.matchCategories.${category.match_category}`)}</p>
                  </div>
                ...
                {categoryRegistrations.length === 0 ? (
                  <p className="text-sm text-slateGray mb-4">{t("tournamentDetail.noRegistrations")}</p>
                ) : (
                      ...
                      return (
                        <div key={r.id} className={"flex items-center justify-between px-4 py-2.5 " + (i !== categoryRegistrations.length - 1 ? "border-b border-black/5" : "")}>
                          <span className="text-ink text-sm">
                            {player?.full_name || t("tournamentDetail.unknownPlayer")}
                            {isDoublesLike && (
                              <span className="text-xs text-slateGray"> · {partnerName ? t("tournamentDetail.pairedWith", { name: partnerName }) : t("tournamentDetail.seekingPartner")}</span>
                            )}
                          </span>
                          {!isLocked && (
                            <button onClick={() => handleRemovePlayer(r)} className="text-xs text-slateGray hover:text-shuttle transition">{t("common.actions.delete")}</button>
                          )}
                        </div>
                      );
                    })}
                ...
                {isLocked ? (
                  <p className="text-xs text-slateGray">{t("tournamentDetail.rosterLocked")}</p>
                ) : isFull ? (
                  <p className="text-xs text-shuttle">{t("tournamentDetail.capacityReached")}</p>
                ) : pending ? (
                  <div>
                    {!pending.awaitingPartner ? (
                      <div className="flex items-center justify-between bg-courtLine/60 rounded-lg px-4 py-3">
                        <span className="text-sm text-ink">{t("tournamentDetail.partnerQuestion", { name: pending.playerName })}</span>
                        <div className="flex gap-2">
                          <button onClick={() => handleRegisterSolo(category.id, pending.playerId)} className="text-xs px-3 py-1.5 rounded-lg bg-white border border-black/10 hover:border-court transition">{t("tournamentDetail.seekingPartnerButton")}</button>
                          <button onClick={() => setPendingRegistration((prev) => ({ ...prev, [category.id]: { ...pending, awaitingPartner: true } }))} className="text-xs px-3 py-1.5 rounded-lg bg-shuttle text-white hover:bg-shuttle/90 transition">{t("tournamentDetail.hasPartnerButton")}</button>
                          <button onClick={() => setPendingRegistration((prev) => { const next = { ...prev }; delete next[category.id]; return next; })} className="text-xs text-slateGray hover:text-shuttle transition">{t("common.actions.cancel")}</button>
                        </div>
                      </div>
                    ) : (
                      <div>
                        <p className="text-sm text-ink mb-2">{t("tournamentDetail.partnerFor", { name: pending.playerName })}</p>
                        <input
                          type="text"
                          value={partnerSearchByCategory[category.id] || ""}
                          onChange={(e) => setPartnerSearchByCategory((prev) => ({ ...prev, [category.id]: e.target.value }))}
                          placeholder={t("tournamentDetail.searchPartnerPlaceholder")}
                          ...
                          <div className="border border-black/5 rounded-lg mt-2 overflow-hidden">
                            {partnerSearchResults.map((p, i) => (
                              <div key={p.id} className={"flex items-center justify-between px-4 py-2 " + (i !== partnerSearchResults.length - 1 ? "border-b border-black/5" : "")}>
                                ...
                                <button onClick={() => handleRegisterPair(category.id, pending.playerId, p.id)} className="text-xs text-shuttle hover:text-shuttle/70 transition font-medium">{t("tournamentDetail.addPairButton")}</button>
                              </div>
                            ))}
                          </div>
                        )}
                        <button onClick={() => setPendingRegistration((prev) => { const next = { ...prev }; delete next[category.id]; return next; })} className="text-xs text-slateGray hover:text-shuttle transition mt-2">{t("common.actions.cancel")}</button>
                      </div>
                    )}
                  </div>
                ) : (
                  <div>
                    <input
                      ...
                      placeholder={t("tournamentDetail.searchPlayerPlaceholder")}
                      className="w-full border border-black/10 rounded-lg px-4 py-2 text-sm outline-none focus:border-court"
                    />
                ...
                {!isLocked && isDoublesLike && pendingTeams.length > 0 && (
                  <div className="mt-4 bg-yellow-50 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-sm font-semibold text-ink">{t("tournamentDetail.seekingPartnerTitle", { count: pendingTeams.length })}</h3>
                      <button onClick={() => handleAutoPairAll(category.id)} className="text-xs px-3 py-1.5 rounded-lg bg-shuttle text-white hover:bg-shuttle/90 transition">{t("tournamentDetail.autoFormPairs")}</button>
                    </div>
                    ...
                    >
                      {t("tournamentDetail.createPairFromSelected")}
                    </button>

                    {oddOneOut && (
                      <div className="border-t border-black/10 pt-3">
                        <p className="text-sm text-ink mb-2">
                          {players.find((p) => p.id === oddOneOut.player_id_1)?.full_name || "—"} — <span className="text-shuttle font-medium">{t("tournamentDetail.unpaired")}</span>
                        </p>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          >
                            <option value="">{t("tournamentDetail.assignPartnerPlaceholder")}</option>
                            {otherPendingForOddOneOut.map((team) => (
                              <option key={team.player_id_1} value={team.player_id_1}>
                                {t("tournamentDetail.optionUnpaired", { name: players.find((p) => p.id === team.player_id_1)?.full_name })}
                              </option>
                            ))}
                            {completeTeams.flatMap((team) => [
                              <option key={team.player_id_1} value={team.player_id_1}>
                                {t("tournamentDetail.optionPaired", { name: players.find((p) => p.id === team.player_id_1)?.full_name })}
                              </option>,
                              <option key={team.player_id_2!} value={team.player_id_2!}>
                                {t("tournamentDetail.optionPaired", { name: players.find((p) => p.id === team.player_id_2)?.full_name })}
                              </option>,
                            ])}
                          </select>
                          <button
                            onClick={() => {
                              ...
                              if (targetInPair && !window.confirm(t("tournamentDetail.confirm.breakExistingPair"))) return;
                          ...
                          >
                            {t("tournamentDetail.assignButton")}
                          </button>
                          <button
                          ...
                          >
                            {t("tournamentDetail.removeFromCategory")}
                          </button>
                        </div>
                      </div>
                    )}
                ...
                <div className="mt-5 pt-5 border-t border-black/5">
                  {isLocked ? (
                    isMaFormat ? (
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          <h3 className="text-sm font-semibold text-ink">{t("tournamentDetail.bracketTitle", { format: t(`common.formats.${tournament.format}`) })}</h3>
                          <div className="flex items-center gap-3">
                            ...
                            >
                              {showLeaderboard[category.id] ? t("tournamentDetail.hideLeaderboard") : t("tournamentDetail.leaderboard")}
                            </button>
                            <button onClick={() => handleDeleteBracket(category)} className="text-xs text-shuttle hover:text-shuttle/70 transition">{t("tournamentDetail.deleteBracketButton")}</button>
                          </div>
                        </div>
                          ...
                          return (
                            <div className="space-y-5">
                                          ...
                                          return (
                                            <div key={m.id} className="bg-white border border-black/10 rounded-lg px-3 py-2 space-y-2">
                                              ...
                                              <div className="flex items-center gap-2">
                                                <button onClick={() => handleUpdateMaMatch(m, sel.a1, sel.a2, sel.b1, sel.b2)} className="text-xs px-2 py-1 rounded bg-court text-white hover:bg-court/90 transition">{t("common.actions.save")}</button>
                                                <button onClick={cancelEditMatch} className="text-xs text-slateGray hover:text-shuttle transition">{t("common.actions.cancel")}</button>
                                              </div>
                                            </div>
                                          );
                                        }
                                        ...
                                        return (
                                          <div key={m.id} className="flex flex-col gap-1.5 bg-courtLine/60 rounded-lg px-3 py-2">
                                            <div className="flex items-center justify-between flex-wrap gap-y-2">
                                              <span className="text-xs text-slateGray w-16 shrink-0">{t("bracketDisplay.court", { number: m.group_number })}</span>
                                            ...
                                            {isBestOf3 && (
                                              <div className="flex items-center gap-3 flex-wrap">
                                                {playedSets.map((s) => (
                                                  <span key={s.set_number} className="text-xs text-slateGray">{t("bracketDisplay.setScore", { number: s.set_number, a: s.team_a_score, b: s.team_b_score })}</span>
                                                ))}
                                                {!isDecided && (
                                                  <div className="flex items-center gap-1.5">
                                                    <span className="text-xs text-slateGray">{t("bracketDisplay.setNumber", { number: nextSetNumber })}</span>
                                                    <input
                                                      type="number" min={0} max={16}
                                                    ...
                                                    <input
                                                      ...
                                                      placeholder="0"
                                                    />
                                                    <button onClick={() => handleSaveSet(m, nextSetNumber)} className="text-xs px-2 py-1 rounded bg-court text-white hover:bg-court/90 transition">{t("bracketDisplay.saveSet")}</button>
                                                  </div>
                                                )}
                                              </div>
                                            )}
                              ...
                              {isMexicano && currentRoundDecided && maxRound < totalRounds && (
                                ...
                                >
                                  {generatingNextRoundFor === category.id ? t("bracketGeneration.generating") : t("bracketGeneration.generateRoundN", { round: maxRound + 1 })}
                                </button>
                              )}
                              {maxRound >= totalRounds && currentRoundDecided && (
                                <p className="text-sm font-semibold text-court">{t("tournamentDetail.tournamentFinished")}</p>
                              )}
                            </div>
                          );
                        })()}
                            ...
                            <div key={groupNumber ?? "single"}>
                              {groupNumber !== null && (
                                <div className="flex items-center justify-between mb-2">
                                  <p className="text-sm font-semibold text-court">{t("tournamentDetail.groupLabel", { number: groupNumber })}</p>
                                  <button
                                    onClick={() => setEditingGroupComposition((prev) => ({ ...prev, [editKey]: !prev[editKey] }))}
                                    className="text-xs text-shuttle hover:text-shuttle/70 transition"
                                  >
                                    {isEditingComposition ? t("tournamentDetail.done") : t("tournamentDetail.editComposition")}
                                  </button>
                                </div>
                              )}
                              ...
                              {isEditingComposition && (
                                <div className="border border-black/10 rounded-lg p-3 mb-3 space-y-1.5 bg-courtLine/40">
                                  {groupTeamIds.length === 0 ? (
                                    <p className="text-xs text-slateGray">{t("tournamentDetail.groupEmpty")}</p>
                                    ...
                                    <div key={teamId} className="flex items-center justify-between text-sm bg-white rounded-lg px-3 py-1.5">
                                      ...
                                      {otherGroupNumbers.length > 0 && (
                                        ...
                                        >
                                          <option value="">{t("tournamentDetail.moveToPlaceholder")}</option>
                                          {otherGroupNumbers.map((g) => <option key={g} value={g}>{t("tournamentDetail.groupLabel", { number: g })}</option>)}
                                        </select>
                                      )}
                                    </div>
                                  ))}
                              ...
                              {groupTeamIds.length > 0 && roundsForGroup.length === 0 && (
                                <p className="text-xs text-slateGray">{t("tournamentDetail.groupTooSmall")}</p>
                              )}
                                        ...
                                        {roundMatches.map((m) => {
                                          ...
                                          if (isBye) {
                                            return (
                                              <div key={m.id} className="flex items-center justify-between text-sm bg-courtLine/60 rounded-lg px-3 py-2">
                                                ...
                                                <span className="text-xs text-slateGray">{t("tournamentDetail.byeAdvances")}</span>
                                              </div>
                                            );
                                          }
                                          ...
                                          return (
                                            <div key={m.id} className="flex items-center gap-2 text-sm bg-white border border-black/10 rounded-lg px-3 py-2 flex-wrap">
                                              ...
                                              <button onClick={() => handleUpdateMatch(m, sel.teamA, sel.teamB)} className="text-xs px-2 py-1 rounded bg-court text-white hover:bg-court/90 transition">{t("common.actions.save")}</button>
                                              <button onClick={cancelEditMatch} className="text-xs text-slateGray hover:text-shuttle transition">{t("common.actions.cancel")}</button>
                                            </div>
                                          );
                                        })}
                              ...
                              {isOlympicFormat && olympicThirdPlaceMatch && (() => {
                                ...
                                return (
                                  <div className="mt-4">
                                    <p className="text-xs font-medium text-slateGray mb-1.5">{t("tournamentDetail.thirdPlaceMatch")}</p>
                                        ...
                                        {!isDecided && !isBestOf3 && (
                                          <div className="flex items-center gap-2 ml-3">
                                            ...
                                            <input
                                              ...
                                              placeholder="0"
                                            />
                                            <button onClick={() => handleSaveScore(tpMatch)} className="text-xs px-2 py-1 rounded bg-court text-white hover:bg-court/90 transition">{t("bracketDisplay.saveResult")}</button>
                                          </div>
                                        )}
                                      </div>
                                          ...
                                          {!isDecided && (
                                            <div className="flex items-center gap-1.5">
                                              ...
                                              <input
                                                ...
                                                placeholder="0"
                                              />
                                              <button onClick={() => handleSaveSet(tpMatch, nextSetNumber)} className="text-xs px-2 py-1 rounded bg-court text-white hover:bg-court/90 transition">{t("bracketDisplay.saveSet")}</button>
                                            </div>
                                          )}
                                        </div>
                                      )}
                              ...
                              {isOlympicFormat && !olympicIsChampionDecided && olympicCurrentRoundDecided && olympicCurrentRoundMatches.length > 1 && (
                                ...
                                >
                                  {generatingNextRoundFor === category.id ? t("bracketGeneration.generating") : t("bracketGeneration.generateNextRound")}
                                </button>
                              )}
                            </div>
                          );
                    ...
                    <p className="text-xs text-shuttle">{t("tournamentDetail.notAllPaired")}</p>
                  ) : isMaFormat ? (
                    participantCount < 4 || participantCount % 4 !== 0 ? (
                      <p className="text-xs text-shuttle">
                        {t("bracketGeneration.errors.notMultipleOfFour", { count: participantCount })}
                      </p>
                    ) : (() => {
                      const isMexicano = tournament.format === "mexicano";
                      const mode = mexicanoSeedingMode[category.id] ?? "auto";
                          ...
                          <div className="mb-4 space-y-3">
                            {isMexicano && (
                              <div className="flex gap-4">
                                {([
                                  { value: "auto", label: t("bracketGeneration.seeding.auto") },
                                  { value: "random", label: t("bracketGeneration.seeding.random") },
                            ...
                            <div>
                              <label className="text-xs text-slateGray mb-1 block">{t("bracketGeneration.roundsCount")}</label>
                              <input
                                type="number" min={1}
                                value={roundsValue}
                            ...
                            <p className="text-xs text-slateGray">{t("bracketGeneration.playersToCourts", { count: participantCount, courts: courtsCount })}</p>
                          </div>
                          <button
                            ...
                            className="px-5 py-2.5 rounded-xl bg-court text-white font-semibold hover:bg-court/90 transition disabled:opacity-50"
                          >
                            {generatingCategoryId === category.id ? t("bracketGeneration.generating") : t("bracketGeneration.generateBracket")}
                          </button>
                        </div>
                      );
                    })()
                    ...
                    (() => {
                      ...
                      const previewSizes = isGroups
                        ...
                        : [];
                      const participantWordKey = isDoublesLike ? "pairs" : "players";
                      const previewText = previewSizes.length
                        ? previewSizes.every((s) => s === previewSizes[0])
                          ? t("bracketGeneration.groupsPreviewEven", { count: participantCount, word: t(`bracketGeneration.participantWord.${participantWordKey}`), groups: previewSizes.length, size: previewSizes[0], groupWord: t(previewSizes.length === 1 ? "bracketGeneration.groupWord.one" : "bracketGeneration.groupWord.few") })
                          : t("bracketGeneration.groupsPreviewUneven", { count: participantCount, word: t(`bracketGeneration.participantWord.${participantWordKey}`), groups: previewSizes.length, sizes: previewSizes.join(", ") })
                        : "";
                            ...
                            <div className="mb-4 space-y-3">
                              <div className="flex gap-4">
                                {([
                                  { value: "auto", label: t("bracketGeneration.seeding.auto") },
                                  { value: "random", label: t("bracketGeneration.seeding.random") },
                                  { value: "manual", label: t("bracketGeneration.seeding.manual") },
                              ...
                              {mode === "manual" && (
                                <div className="flex items-end gap-3">
                                  <div>
                                    <label className="text-xs text-slateGray mb-1 block">{t("bracketGeneration.groupsManual.basisLabel")}</label>
                                    <select
                                      value={basis}
                                      onChange={(e) => setManualBasis((prev) => ({ ...prev, [category.id]: e.target.value as "count" | "size" }))}
                                      className="border border-black/10 rounded-lg px-3 py-2 text-sm outline-none focus:border-court bg-white"
                                    >
                                      <option value="size">{t("bracketGeneration.groupsManual.basisSize")}</option>
                                      <option value="count">{t("bracketGeneration.groupsManual.basisCount")}</option>
                                    </select>
                                  </div>
                                  <div>
                                    <label className="text-xs text-slateGray mb-1 block">{basis === "size" ? t("bracketGeneration.groupsManual.playersPerGroup") : t("bracketGeneration.groupsManual.groupsCount")}</label>
                                    <input
                                      type="number"
                                      min={1}
                                      value={rawValue}
                          ...
                          {isOlympic && (
                            <div className="mb-4 space-y-3">
                              ...
                              <label className="flex items-center gap-1.5 text-sm text-ink cursor-pointer">
                                ...
                                />
                                {t("bracketGeneration.thirdPlaceMatch")}
                              </label>
                                  ...
                                  {Array.from({ length: olympicSize }, (_, posIdx) => {
                                    ...
                                    return (
                                      <div key={posIdx} className="flex items-center gap-2">
                                        <span className="text-xs text-slateGray w-20 shrink-0">{t("bracketGeneration.slotPosition", { position: posIdx + 1 })}</span>
                                        <select
                                          value={currentValue}
                                          onChange={(e) => {
                              ...
                              <p className="text-xs text-slateGray">
                                {t("bracketGeneration.olympicPreview", {
                                  count: participantCount,
                                  word: t(isDoublesLike ? "bracketGeneration.participantWord.pairs" : "bracketGeneration.participantWord.players"),
                                  size: olympicSize,
                                })}
                    ...
                    <p className="text-xs text-slateGray">
                      {isDoublesLike
                        ? t("bracketGeneration.needAtLeastTwoPairs")
                        : t("bracketGeneration.needAtLeastTwoPlayers")}
                    </p>
                  )}
                </div>
              </div>
