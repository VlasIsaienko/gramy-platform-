  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-courtLine px-6">
      <form onSubmit={handleSubmit} className="bg-white rounded-xl p-8 shadow-sm w-full max-w-sm space-y-4">
        <h1 className="text-2xl font-display font-bold text-court mb-2">{t("title")}</h1>
        <div>
          <label className="text-sm text-slateGray mb-1 block">{t("email")}</label>
          <input
            type="email"
            value={email}
        ...
        <div>
          <label className="text-sm text-slateGray mb-1 block">{t("password")}</label>
          <input
            type="password"
            value={password}
